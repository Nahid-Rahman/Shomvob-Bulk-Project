...
(function () {
      ...
      detailsHtml = `
        <div class="field-row" style="margin-top:14px">
          <div class="field">
            ...
            <div class="seg seg-fill" id="ltSandwichSeg" role="group" aria-label="Sandwich rule">
              <button type="button" data-val="yes" aria-pressed="${f.sandwichRuleEnabled}">Yes</button>
              <button type="button" data-val="no" aria-pressed="${!f.sandwichRuleEnabled}">No</button>
            </div>
          </div>
          <div class="field">
            <label>Bridge</label>
            <div class="seg seg-fill" id="ltBridgeSeg" role="group" aria-label="Bridge">
              <button type="button" data-val="yes" aria-pressed="${f.isBridge}">Yes</button>
              <button type="button" data-val="no" aria-pressed="${!f.isBridge}">No</button>
            </div>
          </div>
        </div>
        ${
        ...
        <div class="field-row" style="margin-top:14px">
          <div class="field">
            <label for="ltSandwichMode">Sandwich Mode</label>
            <select id="ltSandwichMode">
              <option value="optional" ${f.sandwichMode === "optional" ? "selected" : ""}>Optional</option>
              <option value="direct_cut" ${f.sandwichMode === "direct_cut" ? "selected" : ""}>Direct Cut</option>
            </select>
          </div>
          <div class="field">
            <label>Include Weekend</label>
            <div class="seg seg-fill" id="ltSandwichWeekendSeg" role="group" aria-label="Include weekend">
              <button type="button" data-val="yes" aria-pressed="${f.sandwichIncludeWeekend}">Yes</button>
              <button type="button" data-val="no" aria-pressed="${!f.sandwichIncludeWeekend}">No</button>
            </div>
          </div>
        </div>
        <div class="field" style="margin-top:14px; max-width:calc(50% - 8px)">
          <label>Include Holiday</label>
          <div class="seg seg-fill" id="ltSandwichHolidaySeg" role="group" aria-label="Include holiday">
            <button type="button" data-val="yes" aria-pressed="${f.sandwichIncludeHoliday}">Yes</button>
            <button type="button" data-val="no" aria-pressed="${!f.sandwichIncludeHoliday}">No</button>
          </div>
        </div>`
            : ""
        }
        ${
          f.isBridge
            ? `
        <div class="field" style="margin-top:14px; max-width:calc(50% - 8px)">
          <label for="ltBridgeMode">Bridge Mode</label>
          <select id="ltBridgeMode">
            <option value="direct" ${f.bridgeMode === "direct" ? "selected" : ""}>Direct</option>
            <option value="optional" ${f.bridgeMode === "optional" ? "selected" : ""}>Optional</option>
          </select>
        </div>`
            : ""
        }
  ...
  function wireLeaveTypeEvents() {
    ...
    const sandwichSeg = $("#ltSandwichSeg");
    if (sandwichSeg) $all("button", sandwichSeg).forEach((btn) => btn.addEventListener("click", () => { snapshotName(); leaveType.fields.sandwichRuleEnabled = btn.dataset.val === "yes"; rerender(); }));
    const bridgeSeg = $("#ltBridgeSeg");
    if (bridgeSeg) $all("button", bridgeSeg).forEach((btn) => btn.addEventListener("click", () => { snapshotName(); leaveType.fields.isBridge = btn.dataset.val === "yes"; rerender(); }));
    const sandwichModeSelect = $("#ltSandwichMode");
    if (sandwichModeSelect) sandwichModeSelect.addEventListener("change", (e) => { snapshotName(); leaveType.fields.sandwichMode = e.target.value; rerender(); });
    const sandwichWeekendSeg = $("#ltSandwichWeekendSeg");
    if (sandwichWeekendSeg) $all("button", sandwichWeekendSeg).forEach((btn) => btn.addEventListener("click", () => { snapshotName(); leaveType.fields.sandwichIncludeWeekend = btn.dataset.val === "yes"; rerender(); }));
    const sandwichHolidaySeg = $("#ltSandwichHolidaySeg");
    if (sandwichHolidaySeg) $all("button", sandwichHolidaySeg).forEach((btn) => btn.addEventListener("click", () => { snapshotName(); leaveType.fields.sandwichIncludeHoliday = btn.dataset.val === "yes"; rerender(); }));
    const bridgeModeSelect = $("#ltBridgeMode");
    if (bridgeModeSelect) bridgeModeSelect.addEventListener("change", (e) => { snapshotName(); leaveType.fields.bridgeMode = e.target.value; rerender(); });
