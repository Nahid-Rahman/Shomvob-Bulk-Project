  ...
  function lastSessionNoticeHtml() {
    ...
    return `
      <div class="validation-banner" style="margin-top:0; margin-bottom:16px;">
        ${iconWarn()}
        <div>You were connected to <strong>${last.companyName}</strong> on ${ENVIRONMENTS[last.env] ? ENVIRONMENTS[last.env].label : last.env} before — sign in with the same company account to pick up where you left off.</div>
      </div>
    `;
  }
  ...
  function holidayCalendarTemplate() {
    ...
    return `
      <div class="section">
        ...
        <span class="error-text" id="hcError">${holidaySync.error}</span>
        ${holidaySync.ok ? `<div style="display:flex; gap:9px; align-items:center; margin-top:10px; color:var(--success); font-size:13px; font-weight:600;">${iconCheck()}${holidaySync.ok}</div>` : ""}
      </div>
    `;
  }
      ...
      <div class="section">
        ...
        <div class="field-row" style="flex-wrap:wrap">
          <div class="field" style="max-width:110px">
            ...
            <input type="number" id="ssBasicPct" value="${f.basicSalaryPercentage}" min="0" max="100" />
          </div>
          ${f.components
            .map(
              (c, i) => `
