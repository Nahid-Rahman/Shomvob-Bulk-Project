...
(function () {
  ...
  function branchTemplate() {
    ...
    const geoFieldsHtml = f.isGeolocation
      ? `
        <div class="field-row" style="margin-top:14px">
          <div class="field"><label for="brLat">Latitude</label><input type="text" id="brLat" value="${escapeHtml(f.latitude)}" /></div>
          <div class="field"><label for="brLng">Longitude</label><input type="text" id="brLng" value="${escapeHtml(f.longitude)}" /></div>
        </div>
        <div class="field" style="margin-top:14px; max-width:calc(50% - 8px)"><label for="brRadius">Radius (meters)</label><input type="text" id="brRadius" value="${escapeHtml(f.radiusInMeters)}" /></div>
      `
      : "";
    return `
      <div class="section">
        ...
        <div class="field-row">
          <div class="field"><label for="brOfficeName">Office Name</label><input type="text" id="brOfficeName" value="${escapeHtml(f.officeName)}" /></div>
          <div class="field"><label for="brDistrict">District</label><input type="text" id="brDistrict" value="${escapeHtml(f.district)}" /></div>
        </div>
        <div class="field-row" style="margin-top:14px">
          <div class="field"><label for="brCity">City</label><input type="text" id="brCity" value="${escapeHtml(f.city)}" /></div>
          <div class="field"><label for="brZip">Zip Code</label><input type="text" id="brZip" value="${escapeHtml(f.zipCode)}" /></div>
        </div>
        <div class="field" style="margin-top:14px"><label for="brAddress">Address</label><input type="text" id="brAddress" value="${escapeHtml(f.address)}" /></div>
        <div class="field" style="margin-top:14px; max-width:calc(50% - 8px)">
          <label>Has Geolocation</label>
          <div class="seg seg-fill" id="brGeoSeg" role="group" aria-label="Has geolocation">
        ...
        <span class="error-text" id="brError">${escapeHtml(companyBranch.error)}</span>
        ${companyBranch.ok ? `<div style="display:flex; gap:9px; align-items:center; margin-top:10px; color:var(--success); font-size:13px; font-weight:600;">${iconCheck()}${escapeHtml(companyBranch.ok)}</div>` : ""}
  ...
  async function runDefaultBranches() {
    const fields = generateBranchFields();
    await saveBranch(fields);
    companyBranch.fields = fields;
    companyBranch.ok = "Saved.";
    companyBranch.createdNames.push(fields.officeName);
