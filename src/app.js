    ...
    return `
      <div class="section">
        ...
        <p class="section-note">${
          usingDefaultThree
            ? "This company has Medical, House Rent and Internet Allowance, so this uses exactly those three — Basic 60%, two of them 15% each, the third 10%. Regenerate re-rolls which two get 15%."
            : "Generated from the muggle-friendly magic scroll's own split table — basic and the two components always sum to 100%. Regenerate re-rolls the split and which two components are used."
        }</p>
        <div style="display:flex; flex-wrap:wrap; gap:6px;">
          <span class="tally">Basic <strong>${f.basicSalaryPercentage}%</strong></span>
          ${f.components.map((c) => `<span class="tally">${nameFor(c.salaryComponentId)} <strong>${c.percentage}%</strong></span>`).join("")}
        </div>
